...
struct SymboleAppearance {
    ...
    explicit SymboleAppearance (const int line, const int index) : line (line), index(index) {}
        ...
        for (QChar const& letter : line)
        {
            if (letter.isDigit()) {
                ...
                if (letter != QChar('.') && letter != QChar(' ')) {
                    const SymboleAppearance symboleAppearance(lineIndex,letterIndex);
